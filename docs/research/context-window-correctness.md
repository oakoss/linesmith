...
- `model.id = "claude-sonnet-4-6"`, `session_id = "9287df90-e125-4b6c-b15d-f6de7148f6af"`. Pre-`/resume` `/context` reported `33.8k/200k tokens (17%)`; stdin at `2026-04-27T00:00:54Z` confirmed `pct=17`. Post-`/resume` `/context` reported `48.4k/200k tokens (24%)`; first post-resume stdin at `2026-04-27T00:01:57Z` confirmed `pct=24`. Delta: 0.
- **`session_id` is unchanged across `/resume`.** The same UUID persisted before and after the command — `/resume` continues the existing session rather than creating a new one. This means linesmith sees `/resume` as an ordinary prompt-to-prompt fill increase, not a session boundary event.
- **Transition signature.** pct stepped 17→24 between `00:00:54Z` and `00:01:57Z`. The percentage-implied delta `(24-17)/100 × 200_000 = 14_000` tokens matches the `/context` byte delta `48.4k − 33.8k = 14.6k` to within rounding (the textual `/context` value is rounded to two significant figures), consistent with the resumed conversation history being re-injected. No anomalous spike or drop; the increase is monotonic.
- **No `>100` clamp triggers.** Fill was well within the safe range; the Anthropic #37163 drift condition did not reproduce.

Row 4 notes:

- **Not testable from stdin captures.** Per CC's documented statusline contract, the command fires after assistant responses, not on transport-level errors. In observed sessions to date, no stdin event has appeared during a 429 retry cycle — consistent with CC handling the retry internally without invoking the statusline. Capture files would show only a timing gap between the last pre-429 capture and the first successful post-retry capture, with no intermediate record. Row marked N/A rather than TBD: the absence of a stdin event during a 429 is the expected behavior under the current contract, not a missing data point. If a future CC version changes this — e.g. fires the statusline with a stale `context_window` during retry — re-open the row.
...
- ~~Are 429 responses even visible at the statusline layer, or are they intercepted upstream and never produce a stdin event?~~ **Resolved 2026-04-27 via row 4:** 429s are intercepted upstream; CC retries internally and the statusline never fires during the retry cycle. Row 4 is N/A — untestable from stdin captures.
