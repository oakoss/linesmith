...
| `publish`          | `*/v<X.Y.Z>` tag push; `mode=release`, or `mode=publish` + `tag`     | Mints an OIDC crates.io token, then `cargo release publish --workspace` (skips already-published members, leaf-first). When every member is skipped — the normal state for the second and later tags of one release — `cargo release` exits 2 with `no packages selected`; the step reports success for that case alone, and only when the number of `disabled due to previous publish` warnings equals the number of crates.io-publishable members reported by `cargo metadata` (the same filter `verify-published` applies). Counting rather than pattern-presence is deliberate: a member can drop out of the selection while logging only at `debug`/`trace`, and an existence check would treat one crate's already-published warning as covering a sibling that never published. A short count fails and names the shortfall. Every other non-zero exit still fails, so a red publish run means a real failure — except when a sparse-index outage makes an already-published member look unpublished, which produces a real (and rejected) publish attempt rather than the empty selection; the runbook describes how the log distinguishes them. |
