...
| `publish`          | `*/v<X.Y.Z>` tag push; `mode=release`, or `mode=publish` + `tag`     | Mints an OIDC crates.io token, then `cargo release publish --workspace` (skips already-published members, leaf-first).                                                                                                                                                                                   |
