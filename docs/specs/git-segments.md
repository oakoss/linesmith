...
**Two compaction layers.** `hide_below_cells` is the user-preference layer: it keys on `terminal_width` alone, so it fires only when the terminal itself is narrow. The engine-driven `shrink_to_fit` layer (defined on `Segment` per `segment-system.md` §Layout algorithm) is the layout-pressure layer: when neighboring segments pressure this one — even on a wide terminal — the engine asks `git_branch` for a compact form before dropping it. The compact form sheds both dirty and ahead/behind markers, keeping `icon + label + head` so the user retains the signal-bearing prefix (`head` is the branch name on a normal checkout, the short SHA on detached HEAD, or the symbolic-ref target on unborn HEAD). Generic end-ellipsis truncation still isn't safe (the structured tail would be mangled), so `truncatable` stays `false`; if even the compact form doesn't fit the engine's target, the segment drops whole via priority.
...
- If `head` is `Branch`, resolve the upstream ref via `Reference::remote_tracking_ref_name(Direction::Fetch)`. When a tracking ref is configured, compute `merge_base(head, upstream)` and count ancestors of each tip that are not reachable from the merge base; the resulting `(ahead, behind)` becomes `Some(UpstreamState { ahead, behind, upstream_branch })`. Store `None` when any of: tracking ref unconfigured, tracking ref not present locally (never fetched), repo is shallow (ancestor walks truncate at the shallow frontier and would undercount), or `merge_base` returns `NotFound` (unrelated histories). (Why the manual walk: gix 0.67's `rev_walk().with_pruned([...])` uses a commit-time cutoff that flakes when two commits share a timestamp — a real risk in statusline renders against fast-moving test fixtures.)
