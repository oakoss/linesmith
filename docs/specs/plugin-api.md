...
- Plugin segments cannot define `shrink_to_fit` yet — the host's default returns `None`, so the engine drops a plugin segment whole under layout pressure rather than asking it for a compact form. Script-side `shrink_to_fit` is a future extension (tracked under `lsm-ns0l`) that will be additive: existing plugins keep working unchanged because the trait default stays `None`.
