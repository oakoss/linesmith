...
`limits` is not in either exclusion set: per [ADR-0030](../adrs/0030-model-scoped-usage-arrives-in-a-limits-array.md) it is a modelled field belonging in `KNOWN_BUCKETS`, so it leaves the WARN row's input entirely rather than being excluded from it. Warning on a key `rate_limit_7d_model` reads would train the operator to ignore this check.
...
- 2026-05-25: §Self "Update available" probe rewritten to fetch `/releases?per_page=30` and filter to linesmith **binary** release tags (`lsm-ghxy`). The old `/releases/latest` endpoint resolves to whichever package shipped most recently, which under the Knope per-package release flow (ADR-0027) can be a library release (`linesmith-core/v*` or `linesmith-plugin/v*`) whose version is unrelated to `CARGO_PKG_VERSION`. The new classifier walks the array entries in reverse-chronological order, picks the first `tag_name` that parses as a binary tag (Knope's `linesmith/v*`, release-plz's legacy `linesmith-v*`, or the pre-monorepo bare `v*`), and compares that against local. Library tags and other non-binary entries silently skip (logged at `Level::Debug` via `lsm_debug!`), so a `LINESMITH_LOG=debug` run surfaces what was rejected. Response body cap bumped from 256 KiB → 4 MiB to fit a 30-entry page (~41 KiB × 30 ≈ 1.2 MiB base + 3× headroom for long-form notes); truncation still surfaces as `TransportError` with a "bump UPDATE_PROBE_MAX_BYTES" hint. New `DoctorUpdateProbe::NoBinaryRelease { scanned }` variant fires when the page contains no parseable binary tag — distinct from `TransportError` (couldn't reach GitHub) and `ParseError` (response shape unexpected) so the WARN can lead with the user-actionable releases URL and surface the page-size knob to maintainers. The previous "verbatim string equality fallback" branch is gone: the filter ensures the picked tag parses as semver, so the comparator only sees semver-vs-semver pairs. A malformed local `CARGO_PKG_VERSION` now surfaces as `ParseError` regardless of upstream outcome (moved above the array walk). The previous 2026-04-30 entry documents the single-object probe shape; the array-walking shape supersedes it.
