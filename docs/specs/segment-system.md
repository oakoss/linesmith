...
Per [ADR-0029](../adrs/0029-group-boundary-marker-and-merge-reconciliation.md), `LineEntryItem` carries a `group: Option<bool>` flag that marks **color** grouping, orthogonal to `merge`'s **spacing**. The config marker and builder grouping are implemented: `group` is a typed field, and the builder records membership via a `fuses_left: bool` on `LineItem::Segment` (true when a segment shares a color group with the segment to its left). The render-side group-lead color pass that consumes `fuses_left` is **lsm-p0p2**; until it lands, `group` is parsed and produces grouping but has no visible color effect yet. The two flags are independent right-boundary intents on a segment entry:
...
- 2026-06-10: v0.13. lsm-v6f0 lands the config + builder half of ADR-0029. `group` is now a typed `Option<bool>` field on `LineEntryItem` (not the `extra` bag); the builder threads `group_pending` parallel to `merge_pending` and records membership as `fuses_left: bool` on `LineItem::Segment` (breaking: external constructors/exhaustive matches of the variant must add the field / `..`). `merge = true` implies `group = true` unless `group = false` overrides; `group` on a separator entry warns. The group-lead color render pass that consumes `fuses_left` remains lsm-p0p2, so grouping is produced but not yet colored.
